...
export async function recentMovies() {
  const endpoint = `${URL}discover/movie?sort_by=release_date.asc&page=1&include_adult=false&primary_release_year=2021&${queryKey}`;
...
export async function getMovies(page, genreId, popularity) {
  let endpoint = `${URL}discover/movie?include_adult=false&page=${page}&with_genres=${genreId}&${queryKey}`;
  if (popularity) endpoint += "&sort_by=popularity.desc";
  const requestResponse = await fetch(endpoint)
    .then((response) => response.json())
    .then((data) => data.results);
  return requestResponse;
